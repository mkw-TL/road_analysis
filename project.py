...
def get_vmt_change(state):
  ...
  county = county[['State','County','Estimates Base', 'Pop_2020','Pop_2021','Pop_2022']]
  county.replace(',','',regex=True,inplace=True)
  county[['Estimates Base', 'Pop_2020','Pop_2021','Pop_2022']] = county[['Estimates Base', 'Pop_2020','Pop_2021','Pop_2022']].astype(int)
  ...
  registrations = pd.read_csv('data/Motor_Vehicle_Registrations__by_vehicle_type_and_state_20231109.csv')
  ...
  vmt_perState[columns_to_use] = vmt_perCap[columns_to_use].multiply(np.array(vmts['Million Miles']))
  ...
  for county, population in county_pop.items():
    ...
    vmt_per_capita_county = state_vmt * (population / sum(county_pop[state_county] for state_county in county_pop if state in state_county))

    # Append the results to the list
    vmt_per_capita_list.append((state, county, vmt_per_capita_county))

  # Create a DataFrame from the results
  vmt_per_capita_diff = pd.DataFrame(vmt_per_capita_list, columns=['State', 'County', 'VMT per Capita'])
  ...
  for county, population in county_pop.items():
    ...
    reg_per_capita_county = state_reg * (population / sum(county_pop[state_county] for state_county in county_pop if state in state_county))

    # Append the results to the list
    reg_per_capita_list.append((state, county, reg_per_capita_county))

  # Create a DataFrame from the results
  reg_per_capita_diff = pd.DataFrame(reg_per_capita_list, columns=['State', 'County', 'Reg per Capita'])
  ...
  for vmt_index, vmt_row in vmt_per_capita_diff.iterrows():
    ...
    reg_row = reg_per_capita_diff[
        (reg_per_capita_diff['State'] == vmt_row['State']) & (reg_per_capita_diff['County'] == vmt_row['County'])
    ]
...
def get_vmt_county(county, state, years=3, road_types=[100,100,100,100,100,100] ,num_avs = 500):
    ...
    pop_old.columns = ['State', 'County','2010','2011','2012','2013','2014','2015','2016','2017','2018','2019']
    ...
    # average vmt per av
    avg_vmt_per_car = av_data['ANNUAL TOTAL'].sum() / len(av_data['ANNUAL TOTAL']) / (1000000 * 12)
    ...
    stepwise_fit = auto_arima(vmt_county,d=d,D=D,seasonal=True,m=12,error_action='ignore')
    x = stepwise_fit
...
#     print(fit_seas)
    ...
    model = ARIMA(data, order=fit,seasonal_order=fit_seas)  # Example order, you might need to tune this
    ...
    coef = [-.366482332, -.285876052, .290216209, .206422313, .292864002, -.151311717]
    with_avs = []
    j = 0
    r = 1.05
    y = 1
    ...
    plt.plot(pd.date_range(start='2023-01-01', periods=years*12,freq='m'), forecast, label='Forecast', color='red')
    plt.plot(pd.date_range(start='2023-01-01', periods=years*12,freq='m'), with_avs, label='Forecast with County Roads for ' + str(num_avs) + ' AVs', color='purple')
    plt.plot(pd.date_range(start='2023-01-01', periods=years*12,freq='m'), pred, label='Forecast with State Traffic for ' + str(num_avs) + ' AVs', color='orange')
    plt.xlabel('Year')
    plt.ylabel('VMT in Millions')
    plt.title('ARIMA Model: Monthly VMT of ' + county + ', ' + state + ' From 2010 to 2026')
    plt.legend()
#     plt.show()
